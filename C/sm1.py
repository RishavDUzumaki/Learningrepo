...
class Sce(Scene):
    def construct(self):
        ...
        block2=Polygon([4*s-2.5,0,0],[4*s-2.5,s,0],[5*s-2.5,s,0],[5*s-2.5,0,0]).set_color([RED,PINK,WHITE]).set_fill([RED,PINK,WHITE],opacity=1)
        springl=always_redraw(
            lambda: Line(
                start=block1.get_right(),end=block2.get_left()
                  ).set_opacity(0)
            )
        spring=always_redraw(
            lambda: springf(block1.get_right(),block2.get_left())
        )
